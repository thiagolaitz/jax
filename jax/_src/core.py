...
class JaxprEqnContext:
  ...
  def manager(self):
    with ExitStack() as stack:
      ...
      stack.enter_context(
          add_attributes(
              **self.attributes if self.attributes is not None else None
          )
      )
      yield
...
def add_attributes(**kwargs: str | None):
  ...
  config.update_thread_local_jit_state(
      attributes_context_manager=tuple(
          (v, k) for k, v in new_attributes.items()
      )
  )
  try:
    yield
    ...
    config.update_thread_local_jit_state(
        attributes_context_manager=tuple(
            (v, k) for k, v in prev_attributes.items()
        )
    )
...
def _pp_eqn(eqn, context, settings, params=None) -> pp.Doc:
  ...
  if lhs.format():
    if settings.print_attributes:
      return pp.concat([
          lhs,
          pp.text(" = ", annotation=annotation),
          pp.text(" ", annotation=attributes_annotation),
          *rhs,
      ])
    else:
